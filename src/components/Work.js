import React from "react";

export const Work = () => {
  return (
    <div id="fh5co-work" className="fh5co-bg-dark">
      <div className="container">
        <div className="row justify-content-center animate-box">
          <div className="col-md-8 col-md-offset-2 text-center fh5co-heading">
            <h2>Work</h2>
          </div>
        </div>
        <div className="row">
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-1.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Illustration</span>
              </div>
            </a>
          </div>
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-2.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Brading</span>
              </div>
            </a>
          </div>
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-3.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Illustration</span>
              </div>
            </a>
          </div>
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-4.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Design</span>
              </div>
            </a>
          </div>
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-5.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Website</span>
              </div>
            </a>
          </div>
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-6.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Illustration</span>
              </div>
            </a>
          </div>
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-7.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Brading</span>
              </div>
            </a>
          </div>
          <div className="col-md-3 text-center col-padding animate-box">
            <a
              href="/#"
              className="work"
              style={{
                backgroundImage: `url("static/images/portfolio-8.jpg")`,
              }}
            >
              <div className="desc">
                <h3>Project Name</h3>
                <span>Illustration</span>
              </div>
            </a>
          </div>
        </div>
      </div>
    </div>
  );
};
